...
export async function issueSession(ctx, db, userId) {
  ...
  const sessionId = await repos(db).sessions.create({
    userId,
    tokenHash,
    expiresAt,
...
export async function resolveSession(db, rawToken) {
  ...
  const row = await repos(db).sessions.byTokenHash(tokenHash);
  if (!row) return null;
...
export async function revokeSession(db, sessionId) {
  await repos(db).sessions.revoke(sessionId);
}

export async function revokeAllSessions(db, userId) {
  await repos(db).sessions.revokeAllForUser(userId);
}
