 ...
 * dev`/manual `npm run build` on a developer machine) — see .env.example,
 * which now defaults VITE_USE_CLOUDFLARE_API=true for local dev too
 * (Supabase was retired in M10.4; there is no other backend to fall back to).
 *
 ...
 * below), so a Workers Build of `main` got no flag at all. The flag is now
 * written for ANY Workers-Builds-triggered build; isPreviewBuild is kept
 * only to distinguish preview from production in the log line (and is still
 * separately useful/tested).
 *
